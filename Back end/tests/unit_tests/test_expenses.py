...
def test_expense_generate_dict(app_client):
    """
    GIVEN expense, expense elements instances
    ...
    expense_obj = Expenses.query.filter_by(id=1).first()

    assert expense_obj.generate_dict() == expense_data


def test_util_create_expense(app_client):
    """
    GIVEN create category and supplier utils
    WHEN calling util_create_expense()
    THEN expense is created;
         proper data is returned.
    """
    clear_db()
    returned_expense = util_create_expense(
        "2023-11-01", 1, "Test supplier", "Test category", "Test units"
    )["expense"]
    queried_expense = Expenses.query.filter_by(
        date=datetime.datetime.strptime("2023-11-01", "%Y-%m-%d").date(),
        total=1,
        supplier=Suppliers.query.filter_by(name="Test supplier").all()[0],
        category=Categories.query.filter_by(name="Test category").all()[0],
    ).all()

    assert len(queried_expense) == 1
    assert returned_expense == queried_expense[0]
