    ...
    <table class="table table-bordered text-center table-sm align-middle">
        ...
        <tbody>
            <tr v-for="(row, index) in rows" :key="row">
                ...
                <td>
                    <SelectField @valueSelected="(newValue) => handleProductSelect(index, newValue)" :name="'expenseElement'+index" :preselectedValue="rows[index].product" :options="row.options" divClasses="" />
                    </td>
                    <td>
                    ...
                    <td>
                        <input type="text" class="form-control" :name="'productTotal'+index" :value="rows[index].quantity * rows[index].price" disabled>
                    </td>
                </tr>
                <tr>
                    <td colspan="4">
                        <button type="button" class="btn btn-outline-success" @click="addRow()" :disabled="category === null">+</button>
                    </td>
                    <td colspan="1">
...
const rows = ref(props.prefilledElements.map(element => ({
    options: goodsData.value.map(product => product.product),
    product: computed(() => {
